...
fn reduce<P: Policy>(
    ...
    loop {
        ...
        match reduced {
            Some(candidate) => config = candidate,
            None => break,
        }
    }
...
fn partition(config: &Configuration, n: usize) -> Vec<Delta> {
    ...
    items.chunks(size).map(|c| c.iter().copied().collect()).collect()
}

struct DDMin;

impl Policy for DDMin {
    fn propose(&mut self, config: &Configuration) -> impl Iterator<Item = Delta> {
        let units = config.len();
        successors(Some(2), move |&n| (n < units).then(|| (2 * n).min(units))).flat_map(move |n| {
...
impl Tree {
    ...
    fn present(&self, id: AtomicUnit, config: &Configuration) -> bool {
        !self.leaves_under(id, config).is_empty()
    }

    fn descendants(&self, id: AtomicUnit, config: &Configuration) -> Vec<AtomicUnit> {
        let mut out = Vec::new();
        let mut stack: Vec<AtomicUnit> = self.nodes[&id].children.iter().copied().collect();
        while let Some(n) = stack.pop() {
    ...
    fn deletable(&self, id: AtomicUnit) -> bool {
        self.parent.get(&id).is_some_and(|p| self.nodes[p].kind == Kind::List)
    }
    ...
    // ANCHOR: subsume
    fn can_replace(&self, n: AtomicUnit, d: AtomicUnit, config: &Configuration) -> bool {
        if n == d {
            return false;
        }
        ...
        match self.parent.get(&anchor) {
            Some(p) if self.nodes[p].kind == Kind::List => is_stmt(d_kind),
    ...
    // ANCHOR: live
    fn live(&self, id: AtomicUnit, config: &Configuration) -> bool {
        let node = &self.nodes[&id];
        match node.kind {
            Kind::Token => config.contains(&id),
            Kind::List => self.parent.get(&id).is_some_and(|&p| self.live(p, config)),
            _ => node
                .children
                .iter()
                .filter(|&&c| self.nodes[&c].kind != Kind::List)
    ...
    fn propose(&mut self, config: &Configuration) -> impl Iterator<Item = Delta> {
        ...
        (start..=max).flat_map(move |l| {
            *level = l;
            ...
            let mut inner = new_minimizer();
            inner
                .propose(&here)
                .map(|drop| -> Delta { drop.iter().flat_map(|&id| tree.leaves_under(id, config)).collect() })
    ...
    P: Policy,
{
    fn propose(&mut self, config: &Configuration) -> impl Iterator<Item = Delta> {
        let tree = self.tree;
        let size: HashMap<AtomicUnit, usize> =
            tree.nodes.keys().map(|&id| (id, tree.leaves_under(id, config).len())).collect();

        let mut nodes: Vec<AtomicUnit> = tree
            .nodes
            .keys()
            .copied()
            .filter(|&id| !tree.nodes[&id].children.is_empty() && tree.live(id, config))
            .collect();
        nodes.sort_by(|&a, &b| size[&b].cmp(&size[&a]).then(a.cmp(&b)));
        ...
        for n in nodes {
            ...
            let mut ds: Vec<AtomicUnit> = tree
                ...
                .into_iter()
                .filter(|&d| tree.live(d, config) && tree.can_replace(n, d, config))
                .collect();
            ds.sort_by(|&a, &b| size[&a].cmp(&size[&b]).then(a.cmp(&b)));
            for d in ds {
                let delta: Delta = n_leaves.difference(&tree.leaves_under(d, config)).copied().collect();
                if !delta.is_empty() {
                    cands.push(delta);
                }
            }
            ...
            if tree.nodes[&n].kind == Kind::List {
                let elems: Configuration =
                    tree.nodes[&n].children.iter().copied().filter(|&c| tree.present(c, config)).collect();
                let mut inner = (self.new_minimizer)();
                for drop in inner.propose(&elems) {
                    let delta: Delta = drop.iter().flat_map(|&id| tree.leaves_under(id, config)).collect();
                    if !delta.is_empty() {
                        cands.push(delta);
                    }
                }
...
fn parses(src: &str) -> bool {
    ...
    impl Parser<'_> {
        ...
        fn ident(&mut self) -> bool {
            let ok = self.toks.get(self.pos).is_some_and(|t| is_ident(t));
            self.pos += ok as usize;
            ok
        }
        fn func(&mut self) -> bool {
            self.eat("int") && self.eat("main") && self.eat("(") && self.eat(")") && self.block()
        }
        ...
        fn if_stmt(&mut self) -> bool {
            self.eat("if") && self.eat("(") && self.ident() && self.eat(")") && self.block()
        }
        fn call(&mut self) -> bool {
            self.ident() && self.eat("(") && self.eat(")") && self.eat(";")
        }
        fn reset(&mut self, pos: usize) {
            self.pos = pos;
        }
    }
    let mut p = Parser { toks: src.split_whitespace().collect(), pos: 0 };
...
fn priors(tree: &Tree, sigma: f64) -> HashMap<AtomicUnit, f64> {
    tree.nodes
        .keys()
        .copied()
        .filter(|&n| n != tree.root)
        .map(|n| (n, if tree.deletable(n) { sigma } else { 1.0 }))
        .collect()
}
// ANCHOR_END: priors
...
fn q(tree: &Tree, p: &HashMap<AtomicUnit, f64>, config: &Configuration, n: AtomicUnit) -> f64 {
...
fn pass_prob(tree: &Tree, p: &HashMap<AtomicUnit, f64>, config: &Configuration, d: AtomicUnit) -> f64 {
...
fn expected_gain(tree: &Tree, p: &HashMap<AtomicUnit, f64>, config: &Configuration, d: AtomicUnit) -> f64 {
    tree.leaves_under(d, config).len() as f64 * pass_prob(tree, p, config, d)
}
...
fn best_candidate(
    ...
    let mut cands: Vec<(AtomicUnit, f64)> = tree
        ...
        .keys()
        .copied()
        .filter(|&id| id != tree.root && tree.live(id, config))
        .map(|id| (id, expected_gain(tree, p, config, id)))
        .collect();

    cands.sort_by(|&(a_id, a_gain), &(b_id, b_gain)| {
        b_gain.partial_cmp(&a_gain).unwrap().then(a_id.cmp(&b_id))
    });

    cands.first().filter(|&&(_, gain)| gain > MIN_GAIN).map(|&(id, _)| id)
}
// ANCHOR_END: choose
...
fn update(tree: &Tree, p: &mut HashMap<AtomicUnit, f64>, config: &Configuration, d: AtomicUnit) {
...
fn main() {
    ...
    let crash = tree.nodes.iter().find(|(_, n)| n.label == "crash").map(|(&id, _)| id).unwrap();
    let setup = tree.nodes.iter().find(|(_, n)| n.label == "setup").map(|(&id, _)| id).unwrap();
    ...
    let make_oracle = || {
        ...
        let oracle = move |c: &Configuration| {
            ...
            let ok = c.contains(&crash) && c.contains(&setup) && parses(&src);
            println!("  test {src:?}  ->  {}", if ok { "keep" } else { "reject" });
            if ok { Verdict::Interesting } else { Verdict::NotInteresting }
        };
        (oracle, calls)
    };
    ...
    let hdd = reduce(all.clone(), &hdd_oracle, Hdd { tree: &*tree, new_minimizer: || DDMin, level: 0 });
    println!("HDD        => {:?}  in {} calls\n", render(&tree, &hdd), hdd_calls.get());

    let (hddp_oracle, hddp_calls) = make_oracle();
    let hddp = reduce(
        all.clone(),
        &hddp_oracle,
        Hdd { tree: &*tree, new_minimizer: || ProbDD { probs: HashMap::new(), p0: 0.1 }, level: 0 },
    );
    println!("HDD+ProbDD => {:?}  in {} calls\n", render(&tree, &hddp), hddp_calls.get());

    let (perses_oracle, perses_calls) = make_oracle();
    let perses = reduce(all.clone(), &perses_oracle, Perses { tree: &*tree, new_minimizer: || DDMin });
    println!("Perses     => {:?}  in {} calls\n", render(&tree, &perses), perses_calls.get());

    let (tpdd_oracle, tpdd_calls) = make_oracle();
    let tpdd = reduce(all.clone(), &tpdd_oracle, TPdd::new(&*tree, 0.5));
    println!("T-PDD      => {:?}  in {} calls\n", render(&tree, &tpdd), tpdd_calls.get());

    let minimal = "int main ( ) { crash ( ) ; setup ( ) ; }";
...
impl Builder {
    fn new() -> Builder {
        Builder { nodes: HashMap::new(), next: 0 }
    }
    fn add(&mut self, kind: Kind, label: &'static str, children: Vec<AtomicUnit>) -> AtomicUnit {
        let id = self.next;
        self.next += 1;
        self.nodes.insert(id, Node { kind, label, children });
        id
    }
    ...
    fn list(&mut self, elems: Vec<AtomicUnit>) -> AtomicUnit {
